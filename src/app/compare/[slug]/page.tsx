...
import { getAllComparisonSlugs, getAllComparisons, markdownToHtml } from '@/lib/content';
  ...
  return (
    <article className="py-10 md:py-14">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-400 mb-8">
        ...
        <Link href="/resources" className="hover:text-teal-primary transition-colors">Resources</Link>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-gray-600 truncate max-w-[200px]">{comparison.frontmatter.title}</span>
      </nav>

      <div className="max-w-3xl mx-auto">
        <div className="mb-8">
          <Badge>{getCategoryLabel(comparison.frontmatter.category)}</Badge>
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mt-4 mb-4 leading-tight">
            {comparison.frontmatter.title}
          </h1>
          <div className="flex items-center gap-3 text-sm text-gray-400">
            <time dateTime={comparison.frontmatter.publishedAt}>
              {formatDate(comparison.frontmatter.publishedAt)}
            </time>
            <span>&middot;</span>
            <span>{comparison.frontmatter.readingTime} min read</span>
          </div>
        </div>
